...
The first question in need of an answer in approaching the problem of optimizing a microgrids design and dispatch is the question of the modelling approach itself. Although the dataset of literature derived from the methodology described in 2.2 is biased towards Linear Programming, which was one of the search and filter criteria, it contains multiple employed methodologies. For example \cite{7975049} employ both a Mixed Integer Linear Programming (MILP) model as well as a Genetic Algorithm (GA) and comes to the conclusion that while both deliver accurate and robust results the MILP model is faster. \cite{NEMATI2018944} mostly concur. While the results of the GA were better in two scenarios, it was outperformed by the MILP model in the remaining three.
...
--rolling horizon\\
--robust optimization\\
--stochatic progrmming\\
--parametric programming\\\\

------scope

--microgrid definition\\  
--islanded versus grid-bound\\
--pure dispatch vs allocation and dispatch\\
--types of generation (dispatchable and non-dispatchable, electric vs thermal)\\
--types of storage (electric vs thermal)\\
--topology included\\
--power quality included\\
--demand response included (controllable vs flexible vs critical loads)\\
--duration and stepsize of case studies\\\\


------parameters

--papers with parameters vs papers without\\
--usual breakdown of cost variables\\
--irridiation and other weather data\\
--load data\\
--other parameters\\\\

------commonly used tools

--commercial tools such as DER-CAM and HOMER \\
--solvers (CPLEX etc.)\\
--modelling languages (MATLAB, GAMS etc.)\\
-- open source tools (github reference) but none with interface\\
