...
\begin{equation}
	\begin{split}
		C_{Operation} = \sum_{u=1}^{U}[\sum_{k=1}^{K}(H_u->GEN_k->C_{OpFix} * S * T)\\
	 	+ \sum_{m=1}^{M}(H_u->dGEN_m->C_{opFix} * S * T)]
	\end{split}
\end{equation}
...
\begin{equation}
	\begin{split}
		...
		+ \sum_{k=1}^K(H_u->GEN_k->C_{OpVar} * genS_{u,k,s,t})\\ 
		+ \sum_{m=1}^M(H_u->dGEN_m->C_{OpVar} * dgenS_{u,m,s,t})]
	\end{split}
\end{equation}
