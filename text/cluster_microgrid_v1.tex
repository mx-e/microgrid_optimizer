...
\begin{table}[H]
	...
	\begin{tabular}{lllll}
		...
		Maximum Input Flow  			& information     		& $\%$ 		 			& 30				& \cite{LAUINGER201624}   \\
		\hline
	\end{tabular}
\end{table}

The storage investment options given are two differntly sized Lithium Ion batteries: A 4 KWh option manufactured by Victron Energy and a 13.5 KWh option manufactured by Tesla. While the two generation devices are hypothetical, these storage devices can actually be bought, which leads to lower error due to false assumptions and linearization of non-linear relationships (such as storage size and price). This the main advantage discrete modeling of investment options has over linear modeling.
...
For the base scenario the grid supply is assumed to be capped at a constant value of 500KW, which guarantees that all supply could be met by the grid. It is further assumed that the price of this energy is set at 0.3\euro at any time. The feed in tarrif is assumed to be constant at 0.09\euro and the feed-in capped at a constant 20KW. \todo{back up those numbers}
