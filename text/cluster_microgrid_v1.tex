...
\todo{more here, definition microgrid, more topic introduction, definitions of words, pv, distributed generation}
...
\todo{more here, GERMANY, capacity building, distribute technological know-how}
\todo{what is a linear model, theory, MILP, different optimisation methods}
...
\todo{rundown of all main chapters}
...
\end{figure}	
Although the resettlement process has already begun and the village 'Morschenich-Neu' is under construction several kilometers away, there are as of the date of writing still several hundred inhabitants remaining in Morschenich, including some Syrian war refugees, who have been accomodated in some of the abandonden houses of families who habe already been resettled or who moved away.\todo{source}
\\
...
Hourly irradiation data (in $KWm^{-2}$) for Morschenich is sourced from \cite{pfenningerLongtermPatternsEuropean2016}. It is then used to calculate the capacity factor, e.g. the percentage of usable capacity of the solar panel, based on the lighting. The resulting vector of values is used as the parameter $GEN_1 \hookrightarrow maxFl$, which is the maximum input flow of the photovoltaic system. 
...
\begin{table}[H]
	...
	\begin{tabular}{lllll}
		...
		Installation Cost           & $C_{Cap}$     		& \euro $KW^{-1}$  		& 1300				& Estimate based on \cite{wirthAktuelleFaktenZur2018}, \cite{SolarmoduleEBay2018}, \cite{ModulePriceIndex2018}   \\
		...
		Electric Efficiency         & $EFF_{el}$     		& $\%$  				& 17				& \cite{wirthAktuelleFaktenZur2018}   \\
...
\subsection{Demand}
Demand vectors for the base scenario are sourced from VDI \todo{here VDI stuff with more detail}...
\\
As the load vectors sourced only cover a duration of 24 hours i generate vectors for any larger duration by repeating the values. In addition a gaussian filter is used to randomise the values to a degree for the purpose of representing a variability of daily profiles that could be observed in reality, without shifting the total consumption.
\\
For the base scenario it is assumed that 10 percent of loads can be curtailed at a flat rate of 0.35\euro per KwH. A further 30 percent of total loads are assumed to be shiftable at a flat rate of 0.15\euro per KwH. The remaining 60 percent of loads are deemed essential by the base scenario.

\subsection{Grid Supply}
For the base scenario the grid supply is assumed to be capped at a constant value of 500KW, which guarantees that all supply could be met by the grid. It is further assumed that the price of this energy is set at 0.3\euro at any time. The feed in tariff is assumed to be constant at 0.09\euro and the feed-in capped at a constant 20KW. \todo{back up those numbers}
